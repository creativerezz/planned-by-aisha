...
<script>
  document.getElementById('hon-yr').textContent = new Date().getFullYear();

  const header = document.getElementById('hon-header');

  function reveal(factor) {
    const vh = window.innerHeight || 800;
    const f = factor || 0.92;
    document.querySelectorAll('.reveal:not(.in)').forEach(el => {
      if (el.getBoundingClientRect().top < vh * f) el.classList.add('in');
    });
  }
  ...
  setTimeout(() => reveal(1.4), 220);
  setTimeout(() => reveal(99), 550);

  let menuOpen = false;

  function toggleMenu() {
    menuOpen = !menuOpen;
    document.getElementById('hon-nav').className = menuOpen ? 'hon-nav open' : 'hon-nav';
  }

  function closeMenu() {
    menuOpen = false;
    document.getElementById('hon-nav').className = 'hon-nav';
  }
  ...
  function handleSubmit(e) {
    ...
    if (!name || !email) {
      if (!name)  f.elements['name'].style.borderBottomColor  = '#e0a0a0';
      if (!email) f.elements['email'].style.borderBottomColor = '#e0a0a0';
      return;
    }
    f.style.display = 'none';
    document.getElementById('hon-success').classList.add('visible');
    window.scrollTo({ top: document.getElementById('hon-inquire').offsetTop - 60, behavior: 'smooth' });
  }
</script>
